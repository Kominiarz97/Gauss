...
int SprCzyOznaczony(double **X,double *Y,int wymiar);
//---------------
int main()
{
    ...
    case 2:
        {
            system("cls");
            cout<<"Przed zerowaniem: "<<endl;
            WyswietlDane(Macierz,Wektor,rozmiar);
            system("cls");
            GaussWybKolumn(Macierz,Wektor,rozmiar);
            cout<<"Po zerowaniu: "<<endl;
            WyswietlDane(Macierz,Wektor,rozmiar);
            if(SprCzyOznaczony(Macierz,Wektor,rozmiar)==0)
            {
                cout<<"Uklad nieoznaczony"<<endl;
            }else if(SprCzyOznaczony(Macierz,Wektor,rozmiar)==1)
            {
...
bool SprawdzCzyMozliwe(double **X,int krok)
{
    if(X[krok][krok]<pow(10,-6) && X[krok][krok]>-(pow(10,-6)))
    {
        return false;
    }
    else
...
void ZerujKolumne(double **X,double *Y,int wymiar,int krok)
{
    double p;
    int pole;
...
int SprCzyOznaczony(double **X,double *Y,int wymiar)
{
    if(fabs(X[wymiar-1][wymiar-1])<pow(10,-10) && fabs(Y[wymiar-1])<pow(10,-10))
    {
        return 0;
    }else if(fabs(X[wymiar-1][wymiar-1])<pow(10,-10) && fabs(Y[wymiar-1])>pow(10,-10))
    {
        return 1;
    }else
    {
